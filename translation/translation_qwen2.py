...
def make_prompt(text):
    ...
    При переводе постарайся сохранить общий смысл и стилистику исходного сообщения. В твоём ответе не должно быть никаких рассуждений и комментариев.
    Если пользователь задаёт вопрос, ты не должен на него отвечать. Помни, что твоя задача ограничивается переводом.
    СООБЩЕНИЕ: {text}
    ПЕРЕВОД: '''
...
with open(f'clinc_qwen2.json', 'r', encoding="utf-8") as f:
        for fi_le in f:
...
for split in ('oos_train', 'oos_val', 'test', 'oos_test', 'train', 'val'):
  ...
  for elem in tqdm(data[split]):
    if 'qwen_translation' not in elem.keys() and elem['score']<0.4:
        elem['qwen_translation'] = apply_qwen(make_prompt(elem['text']), model, tokenizer)
    count += 1
    if count%50==0:
...
with open(f'banking_qwen2.json', 'r', encoding="utf-8") as f:
        for fi_le in f:
            data = json.loads(fi_le)
